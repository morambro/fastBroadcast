...
Devices connection setup consists of the following operations:
	\begin{itemize}
		...
		\item After collecting all the couples \begin{center}\texttt{<MAC address,IP address>}\end{center} from all the clients on a map, it sends it back to all the devices, to let them communicate directly.
	\end{itemize}
...
Our application is structured in different units, each performing a specific function. There is a single Activity class\cite{activities}, called \texttt{MainActivity}
